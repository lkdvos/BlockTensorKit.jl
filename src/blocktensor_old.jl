...
# similar
# -------
function Base.similar(t::BlockTensorMap{S}, ::Type{T}=blocktype(t),
                      P::TensorKit.TensorMapSpace{SumSpace{S}}=space(t)) where {T<:AbstractArray,S<:IndexSpace}
...
function TensorKit.isomorphism(::Type{T}, cod::ProductSumSpace{S},
                               dom::ProductSumSpace{S}) where {T<:Number,S}
    cod ≅ dom || throw(SpaceMismatch("codomain $cod and domain $dom are not isomorphic"))
    t = BlockTensorMap(undef_blocks, T, cod, dom)
    
    mat_inds = CartesianIndices((prod(length.(cod.spaces)), prod(length.(dom.spaces))))
    for (I, J) in zip(mat_inds, CartesianIndices(t))
        V = getsubspace(t, J)
        t[J] = I[1] == I[2] ? isomorphism(T, V) : TensorMap(zeros, T, V)
    end
...
Base.size(A::BlockTensorMap, i::Int) = size(parent(A), i)
Base.axes(A::BlockTensorMap) = axes(parent(A))
Base.axes(A::BlockTensorMap, i::Int) = axes(parent(A), i)
Base.eltype(A::Union{BlockTensorMap,Type{<:BlockTensorMap}}) = eltype(blocktype(A))
Base.IndexStyle(::Type{<:BlockTensorMap}) = IndexCartesian()

# scalar indexing yields a TensorMap
function Base.getindex(t::BlockTensorMap{S,N1,N2,A}, I::CartesianIndex{N}) where {S,N1,N2,T,N,A<:AbstractArray{T,N}}
    return getindex(parent(t), I)
end
function Base.getindex(t::BlockTensorMap{S,N1,N2,A},
                       I::CartesianIndex{N}) where {S,N1,N2,T,N,A<:SparseArray{T,N}}
...
function Base.getindex(t::BlockTensorMap{S,N1,N2,A},
                       I::Vararg{Int,N}) where {S,N1,N2,T,N,A<:AbstractArray{T,N}}
    return getindex(parent(t), I)
end
function Base.getindex(t::BlockTensorMap{S,N1,N2,A},
                       I::Vararg{Int,N}) where {S,N1,N2,T,N,A<:SparseArray{T,N}}
    return getindex(t, CartesianIndex(I))
end
function Base.getindex(t::BlockTensorMap{S,N1,N2,A}, i::Int) where {S,N1,N2,A}
    return getindex(t, CartesianIndices(t)[i])
end
# non-scalar indexing yields a BlockTensorMap
function Base.getindex(t::BlockTensorMap{S,N1,N2,A},
                       I::Vararg{Any,N}) where {S,N1,N2,T,N,A<:AbstractArray{T,N}}
    # Base indexing discards dimensions with scalar indices
    I′ = ntuple(N) do i
        I[i] isa Int && return [I[i]]
        I[i] isa Colon && return axes(t, i)
        return I[i]
    end
    return getindex(t, I′...)
end
function Base.getindex(t::BlockTensorMap{S,N1,N2,A},
                       I::Vararg{<:AbstractVector,N}) where {S,N1,N2,T,N,
                                                             A<:AbstractArray{T,N}}
    V = getsubspace(space(t), I...)
    tdst = similar(t, blocktype(t), V)
    for (j, J) in enumerate(CartesianIndices(t)[I...])
        tdst[j] = t[J]
    end
    return tdst
end
...
# scalar setindex!
function Base.setindex!(t::BlockTensorMap{S,N1,N2,A}, v::T, I::Vararg{Int,N}) where {S,N1,N2,T,N,A<:AbstractArray{T,N}}
    return setindex!(t, v, CartesianIndex(I))
end
function Base.setindex!(t::BlockTensorMap{S,N1,N2,A}, v::T,
                        i::Int) where {S,N1,N2,T,N,A<:AbstractArray{T,N}}
    return setindex!(t, v, CartesianIndices(t)[i])
end
function Base.setindex!(t::BlockTensorMap{S,N1,N2,A}, v::T,
                        I::Vararg{Int,N}) where {S,N1,N2,T,N,A<:SparseArray{T,N}}
    return setindex!(t, v, CartesianIndex(I))
...
function Base.setindex!(t::BlockTensorMap{S,N1,N2,A}, v::T,
                        I::CartesianIndex{N}) where {S,N1,N2,T,N,A<:AbstractArray{T,N}}
    return setindex!(parent(t), v, I)
end
# function Base.setindex!(t::BlockTensorMap{S,N1,N2,A}, v::T,
#                         I::CartesianIndex{N}) where {S,N1,N2,T,N,A<:SparseArray{T,N}}
#     return setindex!(parent(t), v, I...)
...
# non-scalar setindex!
function Base.setindex!(t::BlockTensorMap{S,N1,N2,A}, v::BlockTensorMap{S,N1,N2,A},
                        I::Vararg{Any,N}) where {S,N1,N2,T,N,A<:AbstractArray{T,N}}
    return setindex!.(parent(t), parent(v), Ref.(I)...)
end
function Base.setindex!(t::BlockTensorMap{S,N1,N2,A}, v::BlockTensorMap{S,N1,N2,A},
                        I::Vararg{Any,N}) where {S,N1,N2,T,N,A<:SparseArray{T,N}}
    for (j, J) in enumerate(CartesianIndices(t)[I...])
        if haskey(v, j)
            t[J] = v[j]
        end
...
Base.checkbounds(t::BlockTensorMap, I::CartesianIndex) = checkbounds(parent(t), I)
...
Base.eachindex(a::BlockTensorMap) = eachindex(parent(a))
Base.keys(a::BlockTensorMap) = keys(parent(a))
Base.CartesianIndices(a::BlockTensorMap) = CartesianIndices(parent(a))
...
function Base.copy(tsrc::BlockTensorMap)
    return BlockTensorMap(copy(parent(tsrc)), space(tsrc))
end
function Base.deepcopy(tsrc::BlockTensorMap)
    return BlockTensorMap(deepcopy(parent(tsrc)), space(tsrc))
end
...
TensorKit.space(t::BlockTensorMap) = t.cod ← t.dom
TensorKit.space(t::BlockTensorMap, i::Int) = space(t)[i]

TensorKit.numin(t::BlockTensorMap) = numin(typeof(t))
TensorKit.numin(::Type{<:BlockTensorMap{<:Any,<:Any,N}}) where {N} = N
TensorKit.numout(t::BlockTensorMap) = numout(typeof(t))
TensorKit.numout(::Type{<:BlockTensorMap{<:Any,N}}) where {N} = N
TensorKit.numind(t::BlockTensorMap) = numind(typeof(t))
TensorKit.numind(::Type{<:BlockTensorMap{<:Any,N₁,N₂}}) where {N₁,N₂} = N₁ + N₂
...
function TensorKit.similarstoragetype(TT::Type{<:BlockTensorMap}, ::Type{T}) where {T}
    return TensorKit.similarstoragetype(eltype(TT), T)
end
function TensorKit.similarstoragetype(t::BlockTensorMap, ::Type{T}) where {T}
    return TensorKit.similarstoragetype(typeof(t), T)
end
...
function TensorKit.spacetype(::Union{Type{<:BlockTensorMap{S}},<:BlockTensorMap{S}}) where {S}
    return S
end
function TensorKit.sectortype(::Union{Type{<:BlockTensorMap{S}},<:BlockTensorMap{S}}) where {S} 
    return sectortype(S)
end

blocktype(t::BlockTensorMap) = blocktype(typeof(t))
blocktype(::Type{<:BlockTensorMap{S,N1,N2,A}}) where {S,N1,N2,A} = A
