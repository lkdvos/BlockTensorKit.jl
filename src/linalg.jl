...
function TensorKit.compose_dest(A::BlockTensorMap, B::BlockTensorMap)
    T = Base.promote_op(LinearAlgebra.matprod, scalartype(A), scalartype(B))
    V = codomain(A) ← domain(B)
    if A isa SparseBlockTensorMap # only make sparse if both sparse
        return similar(B, T, V)
    else
        return similar(A, T, V)
    end
end
function LinearAlgebra.mul!(C::BlockTensorMap, A::BlockTensorMap, B::BlockTensorMap,
                            α::Number, β::Number)
...
function LinearAlgebra.mul!(C::BlockTensorMap, A::AbstractTensorMap, B::BlockTensorMap,
                            α::Number,
                            β::Number)
    @assert !(C isa SparseBlockTensorMap)
    szB = size(B)
...
function LinearAlgebra.mul!(C::BlockTensorMap, A::BlockTensorMap, B::AbstractTensorMap,
                            α::Number, β::Number)
    @assert !(C isa SparseBlockTensorMap)
    szA = size(A)
    csizeA = TupleTools.getindices(szA, domainind(A))
...
function LinearAlgebra.mul!(C::TensorMap, A::BlockTensorMap, B::BlockTensorMap, α::Number,
                            β::Number)
